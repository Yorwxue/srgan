...
# facenet_model_path = os.path.expanduser(src_path + 'pretrained/inception_resnet_of_facenet.pb')

inception_v3_model_path = os.path.join(src_path, 'pretrained/inception_v3.ckpt')
npz_inception_v3__model_path = os.path.join(src_path, 'pretrained/inception_v3.npz')
...
#             inception_resnet.load_model(facenet_model_path)
#
#             self.face_images_placeholder = tf.get_default_graph().get_tensor_by_name("input:0")
#             self.face_embeddings = tf.get_default_graph().get_tensor_by_name("embeddings:0")
#             self.face_phase_train_placeholder = tf.get_default_graph().get_tensor_by_name("phase_train:0")
#
#     def run(self, image):
#         feed_dict = {self.face_images_placeholder: image, self.face_phase_train_placeholder: False}
#         return self.sess.run(self.face_embeddings, feed_dict=feed_dict)


def inception_v3_model(x, sess, reuse=False):
    ...
    # if not os.path.isfile(inception_v3_model_path):
    #     print("Please download inception_v3 ckpt from : https://github.com/tensorflow/models/tree/master/research/slim")
    #     exit()
    # try:  # TF12+
    #     saver.restore(sess, inception_v3_model_path)
    # except:  # TF11
    #     saver.restore(sess, "inception_v3.ckpt")
    # print("Model Restored")

    ## load model from npz file
    load_params = tl.files.load_npz(name=npz_inception_v3__model_path)
...
def train():
    ...
    t_target_image_224 = tf.image.resize_images(t_target_image, size=[299, 299], method=0, align_corners=False) # resize_target_image_for_vgg # http://tensorlayer.readthedocs.io/en/latest/_modules/tensorlayer/layers.html#UpSampling2dLayer
    t_predict_image_224 = tf.image.resize_images(net_g.outputs, size=[299, 299], method=0, align_corners=False) # resize_generate_image_for_vgg
    ...
    # inception_v3 model
    # ------------------------------------------------------------------------------------------------------------------
    print('embedding of image of target')
    target_network = inception_v3_model(t_target_image_224, sess)
    target_emb = target_network.outputs
    # ------------------------------------------------------------------------------------------------------------------
    print('embedding of generated image')
    predict_network = inception_v3_model(t_predict_image_224, sess, reuse=True)
    predict_emb = predict_network.outputs
    # ------------------------------------------------------------------------------------------------------------------

    # # facenet_model
    # facenet_model = facenet_graph()
    #
    # facenet_target_emb = facenet_model.run(t_target_image_224)
    # facenet_predict_emb = facenet_model.run(t_predict_image_224)
    ...
    vgg_loss = 2e-6 * tl.cost.mean_squared_error(predict_emb, target_emb, is_mean=True)
...
def sharp(img_path):
    ...
    tl.vis.save_image(out[0], save_dir + '/valid_gen.png')
    tl.vis.save_image(valid_lr_img, save_dir + '/valid_lr.png')
    tl.vis.save_image(img, save_dir + '/valid_origin.png')
...
if __name__ == '__main__':
    ...
    # img_path = '/media/clliao/9c88dfb2-c12d-48cc-b30b-eaffb0cbf545/SRGAN_dataset/face_image/test_lr/110223.jpg'
    # img_path = '/media/clliao/9c88dfb2-c12d-48cc-b30b-eaffb0cbf545/SRGAN_dataset/face_image/train_lr/0494.png'
    # sharp(img_path)
